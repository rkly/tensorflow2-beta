...
def error_demo():
    ...
    y = np.random.rand(10)
    df = pd.DataFrame({'features': x, 'features2': x})

    dataset = tf.data.Dataset.from_tensor_slices((dict(df), y))
    dataset = dataset.batch(32)

    fc = tf.feature_column.sequence_categorical_column_with_hash_bucket('features', hash_bucket_size=5,
                                                                        dtype=tf.dtypes.string)
    fc = tf.feature_column.embedding_column(fc, dimension=10)
    fc2 = tf.feature_column.sequence_categorical_column_with_hash_bucket('features2', hash_bucket_size=5,
                                                                        dtype=tf.dtypes.string)
    fc2 = tf.feature_column.embedding_column(fc2, dimension=10)

    sequence_feature_layer = tf.keras.experimental.SequenceFeatures([fc, fc2])

    dataset = dataset.map(lambda x, y: (sequence_feature_layer(x), sequence_feature_layer(y)))
